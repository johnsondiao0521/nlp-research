...
class Dataset():
    def __init__(self, all_text, all_label, batch_size):
        ...
        self.n = math.ceil(len(self.all_label) / batch_size)

        self.cursor = 0

        self.random_idx = [i for i in range(len(self.all_label))]
    ...
    def __next__(self):
        if self.cursor > len(self.all_label):
            return None

        batch_i = self.random_idx[self.cursor: self.cursor + self.batch_size]
        batch_text = [self.all_text[i] for i in batch_i]
        batch_label = [self.all_label[i] for i in batch_i]
